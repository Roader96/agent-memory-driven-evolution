...
哥 2026-09-10 定调：自进化的主航道是"学会哥的习惯、准确完成任务、变聪明"，
砍 skill 只是副航道。本模块从 state.db 原始对话里挖哥从没写下来的隐性偏好。

三类信号（PRELUDE：用户的"编辑/纠正"成本超阈值才触发归纳）：
  correction  纠正信号：哥否定/修改我的产出 = 我做错的（最强学习信号）
  approval    认可信号：哥确认/表扬 = 做对的（强化）
  habit       习惯信号：哥反复出现的祈使/约束 = 稳定工作方式
...
# 纠正：哥在否定/推翻我的东西（PRELUDE 的 user edit）
CORRECTION = [
...
# 习惯祈使：哥反复用的命令式短语（稳定偏好的候选，需跨会话重复才算）
HABIT_PATTERNS = [
