...
**哥硬规则**："要 X 只做 X"——scan_learnings 只负责**报告高频项**，**不要自动创建/提炼/修改 skill**。
...
扫描发现高频
   ↓
报告给哥（LEARNINGS-REVIEW.md + 飞书）
   ↓
**哥决定要不要提炼**（"要"才动）
   ↓
哥说"提炼为 skill" → 才动 skill
```

**❌ 反模式**：scan_learnings 跑出 20 个候选 → 我自动提炼 1 个新 skill（**哥没让**）。
...
哥说："不是这样，应该是 X"
```bash
~/.hermes/scripts/smart_archive.sh "学习-哥纠正XXX" \
  "我之前答错，正确是X。触发：哥说'不是这样'" \
  "incidents"
```
...
- [ ] 哥纠正后 `HermesMemory/incidents/` 有新条目
