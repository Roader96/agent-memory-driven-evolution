---
name: user-skill-evolution
description: 技能自进化系统。提到技能退役、坏引用、提案队列 P0xx 审批时用。
---

# 技能自进化系统（2026-09-10 建成）

借鉴 Ratchet 论文（SkillsBench：LLM 自写技能 +0.0pp vs 人工精选 +16.2pp，瓶颈在生命周期管理）+ mem0（ADD-only）+ agent-memory-loop（提案队列）。**核心铁律：系统只出提案，绝不自动改技能，用户点头才执行。**
...
| run_weekly.py | 编排器，launchd 周日 21:30 `com.user.skill-evolution-weekly`（metrics→cap→curator→followup→librarian→飞书） |
| cap_enforcer.py | 硬 cap=120（用户 2026-09-10 授权）：超额自动软禁用零加载+≥90天+非自建，按体积降序；`--restore <名>` 恢复；动作落 cap_log |
...
11. **metrics 技能名必须读 frontmatter `name` 字段，不是目录名**（hermes 就是这么解析的）：目录 `example-image-gen/` 的 name 是 `image-gen`、大写字母目录的 name 小写。且匹配要大小写不敏感。曾因此误报 3 个健康技能为坏引用（被 reject）。
...
17. **硬 cap 已落地（cap=120，首执行 142→120）**：cap 与限量退役并存——cap 管总量自动砍，curator 每周 5 个做常规收敛；cap 受害者必须挂 ref_loads 回测（禁用后还被加载=误杀，7 天后查）；被 cap 禁用的 pending retire 提案自动标 done。验证器 verify_evolution_pipeline.py（E 组管 cap、F 组管冷库）。
18. **冷库联动（用户指出盲区后加）**：metrics 新增 vault_mentions_90d——扫 ~/HermesMemory 近 90 天 md（daily/projects/preferences/incidents）里技能名出现次数。用途：① cap/curator 退役豁免——冷库有痕迹=知识被激活过（可能描述注入后无 view 照用），宁可误留不误杀，转人工复核 ② librarian 每个错误簇带 vault_solutions（冷库近 180 天关键词命中的历史方案文件），提案优先从已有经验提炼不从零造。**注意噪声**：技能名常和命令名撞（CLI 命令真用了≠技能加载），所以冷库信号只作豁免/提示不作精确用量，短名（<6字符）不匹配。真实案例：一个 CLU 技能被 cap 首砍，冷库 CICD 存档里有它的部署痕迹→自动救回，cap 改砍无痕迹的另一个技能压回上限。
19. **正向成长主航道（用户定调：砍 skill 是副航道，学会用户的习惯才是主航道）**：preference_signals.py 从 state.db 挖三类信号（纠正=用户否定我产出+我当时产出配对/认可/习惯祈使）；preference_miner.py 每周归纳成偏好候选（PRELUDE/CIPHER 论文+emulo 方案，最大 6 条、可执行不许空话、带证据、去重）；approve_pref.py 审批；批准后**由 agent 用 memory/write_file 写进 ~/.hermes/memories/USER.md**（USER 是权威源，脚本无权改人格）；7 天回测纠正信号是否减少。
...
21. **两路并进 + 每日自省（用户定调）**：副航道（防变笨：cap=120/退役/坏引用）和主航道（变聪明：学用户习惯/纠正）必须同时跑，不能只修一个。每日 watchdog 三件事：① daily_summary 润色 prompt 强制 `## 🧠 自我审视` 段（基于 growth 素材：今天用户纠正几次/集中在什么模式/我最强 agent 差在哪/明天具体动作），证据版兜底也带自省段 ② daily_growth_check 纯规则扫描（复用 collect_day，同类纠正近3天≥2次标复发，趋势写 state preference_trend）③ watchdog 成长门禁：无自省段 exit 1 告警。
