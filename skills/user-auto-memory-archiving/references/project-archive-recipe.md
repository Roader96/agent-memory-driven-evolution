...
> 用户规则：**带项目目录的任务收尾后主动存档冷存储，不用用户提醒**。本 recipe 是可复制的执行步骤。

## 0. Vault 判定（先做这个，别写错）

用户的 OB = `~/HermesMemory/`（唯一）。`~/Documents/Obsidian Vault` 不是归档目标。
...
❌ 反模式：只丢一个 `TIMELINE.md` 装全部内容（用户 2026-08-10 指出「里面只有一个 timeline 跟你之前的版本不一样」）。
