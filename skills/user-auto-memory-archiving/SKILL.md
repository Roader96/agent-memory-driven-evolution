---
name: user-auto-memory-archiving
description: Hermes 冷/热记忆双层自动归档 v3 (含 Obsidian vault 后处理)。所有上下文零丢失 + 不塞爆 system prompt + 智能分类 + 实时归档 + 后处理自动。新话题自动建项目子目录，热记忆超 1800 看门狗静默写标记文件，agent 见标记自行迁移压缩。核心原则：所有对话上下文必须存档，无需提醒。
---

# user-auto-memory-archiving v3

## 核心原则
**所有上下文零丢失 + 不塞爆 system prompt + 无需用户提醒 + 可视化冷库 + 激进分层（不要保守方案）**

> **工作流偏好（用户强信号）**：当用户说"评估个时间频率" / "这个还不够" / "你自己存呗" = **默认给激进完整方案**，不要等用户提需求才加量。实现时**直接做"最完整版本"**，而不是"最简版等用户要求再补"。这条偏好多次重复强化，已成本会话工作流硬规则。

## 📚 References

- `references/project-archive-recipe.md` — 项目型任务收尾主动归档 recipe（vault 确认 / 目录结构 / TIMELINE 模板）
- `references/obsidian-vault-setup.md` — Obsidian 装好后"啥也看不到" 4 步排查
- `references/dont-overengineer.md` — "要 X 只做 X" 反模式清单 + 写作陷阱
- `references/scan_learnings-workflow.md` — 4 类学习来源 + scan_learnings.py 行为规范 + 不主动提炼 skill 原则
- `references/error-logging-recipe.md` — log_error 两种 Python 用法 + CLI 用法 + 错误类别建议
- `templates/LEARNING.md` — 学习条目模板
- `templates/ERROR.md` — 错误条目模板
...
调用时**传项目标签**（推荐）：
```bash
smart_archive.sh "标题" "内容" "项目A"   # → projects/项目A/
...
**为什么不直接重试 + 报错**：cron 红 → 飞书推送炸屏 → 用户烦 → 关 cron。**静默 + log 警告**是正确策略（用户规则：「cron失败>3次告警」，所以单次失败不刷屏，但连错3次会触发下一个机制）。
...
用户的 default 模型切成 `kimi-k3 / custom:kimi` 后，审计 cron 不能只看 `last_status: ok`。三件事必须一起查：
...
2. **skill 改名/废弃后要改 job 引用**：`skills: [hermes-auto-memory-archiving]` 是旧名，当前安装是 `user-auto-memory-archiving`；用 `skill_view(name=...)` 验证，别信 cron list 里的字符串。
...
- 导致 `smart_archive.sh "..." "..." "daily"` 被项目名（website-backend 等）劫持，**应该归 daily/ 的归档被丢到 projects/某项目/**
...
- 复测：`"项目A材料"` 匹到 `"我的新玩具项目A小恐龙"`（短词都是 "项目A"）
...
- 详见 `skill-creator` skill 的 BUG 章节

### 4. **smart_archive 标题含日期会重复（2026-06-23 实测）**
- **症状**：传 `"2026-06-23-项目A深度分析"` → 生成 `2026-06-23-2026-06-23-项目A深度分析.md`（日期前缀 + 标题里的日期 = 2 次）
- **原因**：smart_archive.sh 强制加 `日期-` 前缀，没去重
- **修复**：标题里**不要带日期**（让脚本自动加）
  - ✅ 写：`smart_archive.sh "项目A深度分析" "..." "项目A"`
  - ❌ 写：`smart_archive.sh "2026-06-23-项目A深度分析" "..." "项目A"`
...
- **来源**：用户原话 "**死流程你直接执行就行 不用多思考**"（2026-07-24）
- **关联**：见 `references/pitfalls-2026-07-24.md`

### 6. **归档只写 HermesMemory，别写 Documents/Obsidian Vault（2026-08-10 被用户怼）**
- **症状**：用户说"存档到 OB"，我写进了 `~/Documents/Obsidian Vault/Projects/...`，用户打开看不到 → "我的记忆系统肯定是 HermesMemory 啊 这个这么会搞错？"
...
- **硬规则**：**用户的 OB = `~/HermesMemory/`，唯一**；`~/Documents/Obsidian Vault` 不是归档目标，不要写。拿不准就先 `cat "$HOME/Library/Application Support/obsidian/obsidian.json"` 确认 open 的 vault。
- **项目归档结构**（和现有项目一致，不要只丢一个文件）：
  ...
  ```
  ❌ 反模式：把内容塞进 `TIMELINE.md` 就完事（用户说"里面只有一个 timeline 跟你之前的版本好像也不一样"）。
- **写完后**：更新 `INDEX.md`（项目列表 + 统计 + 最后更新），并确认 TIMELINE 里有 `[[日期文件]]` 链接。

### 7. **memory 工具 replace 是「整条替换」，不是「子串替换」（2026-08-13 史诗级事故）**
- **症状**：想给「页面审美」规则尾部加一句门禁，old_text 只写了尾巴「海报有指定主题时按主题走）」，结果整条「2026-08-12 用户的页面审美（赛博朋克蓝科技风=深蓝底+蓝青霓虹+矩阵雨+故障标题+图标多+等宽数字+一屏不滚动）」被 content 覆盖成半截，整套审美标准丢失。
...
- **同期事故二**：热记忆一路涨到 96%（2117/2200）没迁移。自动归档 hook 依赖「每轮自动调用」但实际未生效 → **agent 必须主动兜底**：热记忆 >1800 立即迁移低频项，不等 hook、不问用户。迁移优先迁「详见 skill X」类冗余条目（细节已在 skill，热记忆只留硬规则核心 + 指针）。
...
- **2026-09-07 v2（用户令：告警不推飞书，静默自处理）**：脚本 `wc -m` 直扫真实文件，**stdout 永远静默**（不飞书不通知）；≥1800 只写标记文件 `~/.hermes/data/.memory_overload_pending`（内容=用量+时间戳），<1800 自动清标记。
...
- 症状：用户投诉「这几天一直在用 Hermes，但总结没有实质性东西」「自动更新失败都没记录，你在 improving 啥，形式主义」。
...
- **v4 架构（2026-09-10 重写，唯一入口 = launchd 23:55 `daily_watchdog.sh`）**：
  1. `~/.hermes/scripts/daily_summary_from_db.py` 直读 **state.db**（sessions/messages 表）拉当天**全部** session：用户的每条 user 原话 + assistant 带结论信号（✅/根因/修复/结论/决定…）的消息，兜底最长+最后一条。这是**证据层（零 token，数学可验）**。
  ...
  - **日常静默写 OB，不推飞书**（2026-09-10 用户令「不轰炸，直接看 OB」）：watchdog 跑脚本不带 PUSH_FEISHU；想临时推送才 `PUSH_FEISHU=1 python3 daily_summary_from_db.py`。生成**失败**仍保留 macOS osascript 本机通知（不是飞书）。回归验证器 `~/.hermes/scripts/verify_daily_pipeline.py` 已同步断言「静默无 message_id」。
  5. 润色 hermes -z/chat 调用会产生新 session 塞爆桌面会话列表（用户投诉）：必须用 `hermes chat -Q --oneshot --cli --source daily-polish --query-file <prompt文件>`（专属 source），跑完 `hermes sessions archive --source daily-polish --yes` 软隐藏（archived=1，数据留在 state.db 供 SQL 读，列表不显示）；load_sessions 同时用 source==daily-polish + prompt 标记 `[daily-summary-polish]` 双保险排除。别用 -z "长prompt"（命令行长度/转义坑，且 source=cli 难清理）。历史测试垃圾会话可直接 `UPDATE sessions SET archived=1 WHERE id=...`（独立 oneshot 无父子链才安全）。
- **调度**：launchd `hermes-daily-watchdog` 唯一调度（零 token、睡眠补跑）；watchdog 还静默回填最近 3 天缺失 daily、跑 vault_postprocess + scan_learnings。
- 借鉴的开源项目（用户要求别闭门造车）：mem0(65k⭐, ADD-only 追加不覆盖+agent动作即存)、agent-memory-loop(一行错误日志 id/count/severity/source)、self-improve(Reflector 规则收集→模型提炼)。
...
**触发（用户一提旧事 → 必走，不靠自觉）**：
- 用户提过去某件事 / 项目名 / 人名 / 说「上次那个」「还记得吗」「之前我们」「那个 XX 现在啥情况」
- 答案在历史里、不在当前上下文 → 立即走召回，别凭印象硬答

**检索顺序（session_search 是核心，实测命中率高）**：
1. `session_search(query=用户原话, limit=3)` 搜会话历史
...
- 结尾附 `@session:xxx` link + 档案路径，让用户能点进看全文
...
本 skill 是**唯一**的"长记忆 + 自动归档"入口。同类工作流若有独立 skill 出现，按照下面的合并规则走：

- **`agent-memory-continuity` → absorbed here**（冷/热分层 + `vault_postprocess.py` 脚本与本 skill 完全重叠）
- **`self-improvement` / `.learnings/` → partial absorption**（`.learnings/LEARNINGS.md` / `.learnings/ERRORS.md` 这套 CLI 仍然用，但 **提炼成 skill 的动作**仍归本 skill 触发 `scan_learnings.py`，由 `用户决定要不要提炼` 才动 skill）
...
23:50 cron 任务不是"跑 daily_summary.sh 就完事"，完整流程有 5 步。漏掉最后两步用户看不到总结。
...
- ❌ 错：5 个独立 `hermes send` 调用 → 用户 DM 被刷屏 5 条
- ✅ 对：1 个 `hermes send` 含完整多行消息（上面模板就是 1 个 call）

### ⚠️ Pitfall: `daily_summary.sh` 模板可能不如磁盘版新

用户的 prompt 里给的 `daily_summary.sh` 默认模板是**最简版**（daily 写入 + postprocess + hook status）。但磁盘上的版本从 2026-06-11 起已经升级成**带 `run_py` 重试 + learnings 扫描 + 错误 JSON 读取**的完整版。
...
不要硬写 `feishu:oc_<hex>`——如果用户换过 chat、或者新开了 DM，得用最新那个。
...
### 陷阱 2：过度工程（"要 X 只做 X"）
- **症状**：用户说"加 Y" → 我额外改 B/C/D，重写 skill
- **判断**：
  - 改动 >3 个文件 → ❌ 可能过度
...
**scan_learnings 跑完不动任何 skill**——这是核心约束。**❌ 反模式**：scan_learnings 跑出 20 个候选 → 自动提炼 1 个新 skill（用户没让）。
